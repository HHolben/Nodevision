// Nodevision/ApplicationSystem/Sync/sync-sync-test-two-way.mjs
// This script performs safe two-way SyncTest convergence by pulling peer-only files, pushing local-only files, and storing changed peer files as local conflict copies without overwriting local originals.
...
export function buildTwoWaySyncSelection(plan) {
...
async function pushLocalFile(peerUrl, notebookDir, requestedRelativePath, runtimeRoot) {
  const target = resolveSyncTestTarget(notebookDir, requestedRelativePath);
  ...
  const contentBuffer = await fs.readFile(target.targetPath);
  const sha256 = createHash("sha256").update(contentBuffer).digest("hex");
  const signedRequest = await createSignedFilePush(
    {
  ...
  const body = await postJson(peerUrl, "/api/peer/file-push", signedRequest);
  if (!body || body.ok !== true || !body.saved) {
    throw new Error("Peer file-push response missing save payload");
  }

  const savedRelativePath = validateSyncTestRelativePath(body.saved.relativePath);
  if (savedRelativePath !== requestedRelativePath) {
    throw new Error("Peer reported unexpected saved relativePath");
  }

  return { relativePath: requestedRelativePath, bytes: contentBuffer.length, sha256 };
}
