...
const parametricPreview = parseBasicScad(`x0 = 100;
y0 = 80;
z0 = 10;
r1 = 8;
h1 = 20;
...
union() {
  cube([x0, y0, z0]);
  translate([x1, y1, z0]) cylinder(r = r1, h = h1);
  translate([x2, y2, z0]) cylinder(r = r1, h = h1);
  translate([x3, y3, z0]) cylinder(r = r1, h = h1);
  translate([x4, y4, z0]) cylinder(r = r1, h = h1);
  translate([x5, y5, z0]) cylinder(r = r1, h = h1);
  translate([x6, y6, z0]) cylinder(r = r1, h = h1);
  translate([x1, y1, z0 + h1]) cylinder(r = r2, h = h2);
  translate([x2, y2, z0 + h1]) cylinder(r = r2, h = h2);
  translate([x3, y3, z0 + h1]) cylinder(r = r2, h = h2);
  translate([x4, y4, z0 + h1]) cylinder(r = r2, h = h2);
  translate([x5, y5, z0 + h1]) cylinder(r = r2, h = h2);
  translate([x6, y6, z0 + h1]) cylinder(r = r2, h = h2);
}`);
assert.equal(parametricPreview.objects.length, 13);
assert.deepEqual(parametricPreview.objects[0].params.size, [100, 80, 10]);
const parametricCylinders = parametricPreview.objects.filter((obj) => obj.type === "cylinder");
assert.equal(parametricCylinders.length, 12);
assert.deepEqual(parametricCylinders[0].transform.translate, [10, 10, 10]);
